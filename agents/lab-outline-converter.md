...
1. **Can be built by `/lab-environment-builder`** — Precise enough that another agent can generate Docker services, seed scripts, and check scripts without guessing
...
**Buildability check:** [Can lab-environment-builder generate Docker services, seed scripts, and checks from this? Are all details concrete?]
...
**CRITICAL:** The spec must be specific enough that `/lab-environment-builder` can generate Docker services, seed scripts, and check scripts without making assumptions. The builder's output (the lab environment) must be completable by humans.
...
**Note for builder:** This section is used by `/lab-environment-builder` to generate seed scripts. Be specific.
...
**Note:** These artifacts are validated by check scripts generated by `/lab-environment-builder`. Be precise so the check scripts work correctly.
...
**What it validates:** [Exact conditions the check script will test — be precise so `/lab-environment-builder` can implement this correctly]
...
3. **Testability** — Milestone check must validate exact behaviors described in the stage. Expected output must be shown exactly as it appears. `/lab-environment-builder` uses these checks to generate validation scripts that humans use.
...
6. **Buildability** — Every artifact, seed data item, and check must be concrete enough that `/lab-environment-builder` needs no clarification.

### 4. Ensure Builder Can Create a Completable Lab

For every stage:

- **Buildability:** Specify exact seed data (field names, values), exact artifacts (file names, content), exact check commands with expected output. `/lab-environment-builder` will read this verbatim to generate the lab environment.
...
4. **Testability** — Can `/lab-environment-builder` generate check scripts from milestone specs? ✓ / △ / ✗
...
Next step: 
- If both YES: Run `/lab-instruction-evaluator` to validate spec quality
...
**Buildability (for `/lab-environment-builder`):**
...
- ✓ Spec is ready to pass to `/lab-instruction-evaluator` and `/lab-environment-builder` without revision
